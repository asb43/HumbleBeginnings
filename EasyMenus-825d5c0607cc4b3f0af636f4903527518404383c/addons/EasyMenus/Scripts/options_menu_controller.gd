...
const HSliderWLabel = preload("res://EasyMenus-825d5c0607cc4b3f0af636f4903527518404383c/addons/EasyMenus/Scripts/slider_w_labels.gd")
...
#	gp_look_sens_slider.value_changed.connect(_on_gp_looksens_slider_value_changed)
	...
	sfx_bus_index = AudioServer.get_bus_index(GameOptionsConstants.sfx_bus_name)
	music_bus_index = AudioServer.get_bus_index(GameOptionsConstants.music_bus_name)
...
# Saves the options
func save_options():
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.invert_vertical_axis_key, invert_y_check_button.button_pressed)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.head_bobble_key, headbob_strength)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.mouse_sens_key, mouse_sens)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.gp_looksens_key, gp_looksens)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.windowmode_key_name, window_mode_option_button.selected)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.resolution_index_key_name, resolution_option_button.selected)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.render_scale_key, render_scale_slider.value);
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.gui_scale_key, gui_scale_slider.value);
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.vsync_key, vsync_check_button.button_pressed)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.msaa_2d_key, anti_aliasing_2d_option_button.get_selected_id())
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.msaa_3d_key, anti_aliasing_3d_option_button.get_selected_id())
	
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.sfx_volume_key_name, sfx_volume_slider.hslider.value)
	config.set_value(GameOptionsConstants.section_name, GameOptionsConstants.music_volume_key_name, music_volume_slider.hslider.value)
	
	config.save(GameOptionsConstants.config_file_name)
...
func load_options():
	...
	var invert_y = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.invert_vertical_axis_key, true)
	mouse_sens = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.mouse_sens_key, 0.25)
	gp_looksens = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.gp_looksens_key, 2)
	headbob_strength = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.head_bobble_key, 2)
	
	var sfx_volume = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.sfx_volume_key_name, 1)
	var music_volume = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.music_volume_key_name, 1)
	var window_mode = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.windowmode_key_name, 0)
	var resolution_index = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.resolution_index_key_name, 0)
	var render_scale = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.render_scale_key, 1)
	var gui_scale = config.get_value(GameOptionsConstants.section_name, GameOptionsConstants.gui_scale_key, 1)
